...
# MASTER GENERATOR
# ==========================================
def generate_castle(editor, build_area, rng_seed):
    """Executes the procedural generation pipeline within a specific build_area."""
...
def run_automated_evaluation():
    ...
    global_area = editor.getBuildArea()
    
    # 1. Safety Check: Ensure the user selected a massive area in Minecraft
    if global_area.size.x < 500 or global_area.size.z < 500:
        print("⚠️ WARNING: Your selected build area is too small for the automated test!")
        print("Please go into Minecraft and run a command like:")
        print("/setbuildarea ~-500 ~ ~-500 ~500 ~ ~500")
        print("This will create a 1000x1000 testing zone.")
        return

    test_seeds = [42, 101, 2026, 777, 999, 12345, 54321, 8888, 1111, 3333]
    results = []
    
    print(f"Starting Automated Evaluation over 10 random sub-regions...")
    print(f"Global Build Area Boundary: {global_area.size.x}x{global_area.size.z}")
    
    for idx, seed_val in enumerate(test_seeds):
        random.seed(seed_val) # Seed the picker so the locations are reproducible
        
        # Change the search area to 200 so the algorithm has room to scout
        SEARCH_AREA_SIZE = 200 
        
        max_offset_x = global_area.size.x - SEARCH_AREA_SIZE
        max_offset_z = global_area.size.z - SEARCH_AREA_SIZE
        
        sub_x = global_area.begin.x + random.randint(0, max_offset_x)
        sub_z = global_area.begin.z + random.randint(0, max_offset_z)
        
        # Update the Box creation to use SEARCH_AREA_SIZE
        sub_offset = (sub_x, global_area.begin.y, sub_z)
        sub_size = (SEARCH_AREA_SIZE, global_area.size.y, SEARCH_AREA_SIZE)
        sub_area = Box(sub_offset, sub_size)
        
        print(f"\n--- Running Trial {idx+1}/10 (Seed: {seed_val}) ---")
        print(f"Scouting new sub-region at Coordinates: X:{sub_x}, Z:{sub_z}")
        
        start_time = time.time()
        
        try:
            # 4. Pass the specific sub-area to your modularized generator!
            generate_castle(editor, sub_area, seed_val) 
            
            end_time = time.time()
            elapsed_time = end_time - start_time
            results.append((seed_val, sub_x, sub_z, elapsed_time))
            print(f"Trial {idx+1} completed in {elapsed_time:.2f} seconds.")
            
        except Exception as e:
            print(f"Trial {idx+1} failed with error: {e}")
            results.append((seed_val, sub_x, sub_z, "Failed"))
            
        # Give the Minecraft server 3 seconds to catch its breath before the next massive edit
        time.sleep(3) 

    # 5. Generate the Markdown Table for your paper
    print("\n\n" + "="*75)
    print(" RESULTS TABLE (Copy directly into your paper)")
    print("="*75)
    print("| Trial | Random Seed | Map Coordinates (X, Z) | Generation Time (s) |")
    print("| :---: | :---: | :---: | :---: |")
    
    total_time = 0.0
    success_count = 0
    
    for i, (seed_val, sx, sz, t) in enumerate(results):
        loc_string = f"({sx}, {sz})"
        if isinstance(t, float):
            print(f"| {i+1} | `{seed_val}` | {loc_string} | {t:.2f} |")
            total_time += t
            success_count += 1
        else:
            print(f"| {i+1} | `{seed_val}` | {loc_string} | **{t}** |")
            
    if success_count > 0:
        avg_time = total_time / success_count
        print(f"\n**Average Execution Time:** {avg_time:.2f} seconds across {success_count} successful distinct biomes.")
        print("="*75)
